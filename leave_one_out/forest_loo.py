...
def classify_by(iv, dv, outname="classifier.obj", train_ratio=0.8, show_feat=False, save_classifier=True):
    ...
    cl = nltk.classify.scikitlearn.SklearnClassifier(RandomForestClassifier())
    cl = cl.train(train)
...
if __name__ == "__main__":
    ...
    if local:
        l = [
            leave_one_out(fr.spatial, fr.group, "spatial_loo_forest"),
            leave_one_out(fr.social, fr.group, "social_loo_forest"),
            leave_one_out(fr.hypothetical, fr.group, "hypothetical_loo_forest"),
            leave_one_out(fr.control, fr.group, "control_loo_forest"),
            leave_one_out(fr.temporal, fr.group, "temporal_loo_forest"),
            leave_one_out(flattened.text, flattened.group, "all_loo_tree")
        ]
        print(l)
        df = DataFrame(l)
        ...
        sbatch.load("leave_one_out", [fr.temporal, fr.group, "temporal_loo_forest"])
        sbatch.load("leave_one_out", [fr.spatial, fr.group, "spatial_loo_forest"])
        sbatch.load("leave_one_out", [fr.social, fr.group, "social_loo_forest"])
        sbatch.load("leave_one_out", [fr.hypothetical, fr.group, "hypothetical_loo_forest"])
        sbatch.load("leave_one_out", [fr.control, fr.group, "control_loo_forest"])
        sbatch.load("leave_one_out", [flattened.text, flattened.group, "all_loo_forest"])
        sbatch.launch()
